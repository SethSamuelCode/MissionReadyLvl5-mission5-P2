...
const Header = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (trimmed !== "") {
      ...
      // If not already on marketplace, navigate there
      if (!location.pathname.includes("/marketplace")) {
        navigate(`/marketplace?search=${encodeURIComponent(trimmed)}`);
      } else {

        // If already on marketplace, replace current state and reload
        navigate(`/marketplace?search=${encodeURIComponent(trimmed)}`, {
          replace: true,
        });
  ...
  return (
    <header className={styles.fullHeader}>
      ...
      {/* Top section: logo, search bar, user icons */}
      ...
      {/* Bottom category bar under header */}
